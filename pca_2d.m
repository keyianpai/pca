...
sizes = size(x); %n by m n :feature m: example
figure(1);
scatter(x(1, :), x(2, :));
title('Raw data');
...
[u,s,v]=svd(x);
u ;size(v); %v 45*45
diag(s).*diag(s); %same with s columns of u is same +/-1*
%u = u(:,1);
[0 u(1,1)]'

% -------------------------------------------------------- 
hold on
plot([0 u(1,1)], [0 u(2,1)]);%plot([0 u(1,1),1], [0 u(2,1),-1]);
